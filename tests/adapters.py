...
from cs336_basics.model import SwiGLU, SiLU, silu, MultiHeadSelfAttention, TransformerBlock, TransformerLM
from cs336_basics.layers import Linear, Embedding, RMSNorm, RotaryPositionalEmbedding, softmax, scaled_dot_product_attention
...
def run_embedding(
    ...
    embedding_kayer = Embedding(num_embeddings=vocab_size, embedding_dim=d_model)
    embedding_kayer.load_state_dict({"weight": weights})
    return embedding_kayer(token_ids)
...
def run_rmsnorm(
    ...
    """
    rms_norm_layer = RMSNorm(d_model=d_model, eps=eps)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    return clip_gradients(parameters=parameters, max_l2_norm=max_l2_norm)
